...
class Spreadsheet(object):
		...
		if t == dict:
			for k in data:
				if k == 'Signal type':
					...
					if type(st) == list:
						l = len(st)
						if sc.has_key('Remote-i/o device ID'):
							if not sc.has_key('Serial port'):
								sc['Serial port'] = [None] * l
							if not sc.has_key('Modbus address'):
	...
	def parse(self):
		...
		try:
			...
			pass
		if data.has_key('Remote-io device'):
