#!/usr/bin/env python3
from my_classes import ColorChart,NavigationToolbar,HoverButton,move_obj
...
def plot(fig,axes,lines,leg,from_):
    ...
    def save():
        ...
        with open(file, 'wb') as f: # should be 'wb' rather than 'w'
            pickle.dump((fig,texts,vlines,hlines,arrow_lines), f)
        master.destroy()
    ...
    def interact_title():
        ...
        try:
            ...
            e_title_size = tk.Spinbox(change_title, from_=2, to=30,textvariable=var_title,bd=5,width=5,increment=0.5)
    ...
    def interact_grid():
        ...
        try:
            ...
            for ind,ax in enumerate(axes):
                ...
                grid_width_val.set(str(glx.get_linewidth()))
    ...
    def interact_traces():
        ...
        def edit_traces():
            ...
            for ind,trace in enumerate(trace_entries):
                try:
                    lines[ind].set_linewidth(float(trace_widths[ind].get()))
                    leg.get_lines()[ind].set_linewidth(float(trace_widths[ind].get()))
                    ...
                    if dead_flag !=0:
                        ...
                        line_deads.pop(ind)
                    lb=lines[ind].axes.fill_between(lines[ind].get_data()[0],lines[ind].get_data()[1]-float(trace_deads[ind].get()),lines[ind].get_data()[1]+float(trace_deads[ind].get()),alpha=0.2,facecolor=lines[ind].get_color())
    ...
    try:
        for ts in tss:
            dr = move_obj(ts,fig,None,master)
            dr.connect_obj()
            drs.append(dr)
            texts.append(ts)
    except:pass
    ...
    try:
        for vs in vss:
            vlines.append(vs)
            dv = move_obj(vs,fig,"Vertical",master)
            dv.connect_obj()
            dvs.append(dv)
    except:pass
    ...
    try:
        for hs in hss:
            hlines.append(hs)
            dh = move_obj(hs,fig,"Horizontal",master)
            dh.connect_obj()
            dhs.append(dh)
    except:pass
    ...
    try:
        for ar in ars:
            arrow_lines.append(ar)
            da = move_obj(ar,fig,"Arrow",master)
            da.connect_obj()
            das.append(da)
    except:pass
...
input_csv = tk.Tk()
input_csv.withdraw()
csv = filedialog.askopenfilename(initialdir = "/home",title = "Select csv file",filetypes = (("csv files","*.csv"),("pickle files","*.pickle"),),parent=input_csv)
if not csv:
    sys.exit()
...
if 'csv' in csv:
    ...
    choose_plot=tk.Tk()
    choose_plot.withdraw()
    with open(csv, 'rb') as file:
        figx,t,v,h,a = pickle.load(file)
        fig,axes,lines,leg,tss,vss,hss,ars=plot_existing(figx,t,v,h,a)
    from_='pickle'
    plot(fig,axes,lines,leg,from_)
    choose_plot.mainloop()
